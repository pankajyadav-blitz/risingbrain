...
/**
 * Score bar in the paper header. Reflects the STORED mark for this topic's test
 * (updated only on submit) — `score/total` + %. Shows a neutral "Not attempted"
 * state until the learner has submitted at least once.
 */
export function TopicProgressBar({ topicId, total }: { topicId: string; total: number }) {
  ...
  if (!stored) {
    return (
      <p className="mt-3 text-xs text-muted">Not attempted yet — solve and submit to score.</p>
    );
  }
