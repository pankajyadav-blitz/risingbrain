import { getQuizIndex, getProgressSeed } from "../data";
...
/**
 * The data-driven quiz shell — header + index + paper — split out of the
 ...
 */
export async function QuizWorkspace({
  profileId,
  kinds,
  basePath,
  emptyLabel,
  ...
  /** Which QuizCategory kinds this route shows. */
  kinds: AptKind[];
  /** Route prefix the index and picker navigate within. */
  basePath: string;
  /** Noun used in the "nothing published" message, e.g. "quizzes" / "puzzles". */
  emptyLabel: string;
  ...
  if (categories.length === 0) {
    return (
      <p className="pb-20 text-muted">No {emptyLabel} have been published yet. Check back soon.</p>
    );
  }
    ...
    >
      <SelectedCategoryProvider categories={categories}>
        ...
        <div className="mb-6 lg:shrink-0">
          <CategoryTabs basePath={basePath} />
        </div>
        ...
        <div className="pb-8 lg:flex lg:min-h-0 lg:flex-1 lg:flex-col lg:pb-0">
          ...
          <MobilePicker categories={categories} basePath={basePath} />
