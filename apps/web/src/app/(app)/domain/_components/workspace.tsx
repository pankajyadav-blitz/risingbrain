import { getDomainIndex, getDomainProgressSeed } from "../_data";
import { DomainProgressProvider } from "./progress-provider";
 ...
 * straight through. Two providers are seeded here, once: the selected-subject one
 * that links the tabs, the sidebar and the mobile picker, and the live progress
 * one that links the index's practice counters to the topic's graded state.
 */
export async function DomainWorkspace({
  ...
  const seed = profileId
    ? await getDomainProgressSeed(profileId)
    : { reviewByQuestion: {}, submittedTopics: {} };

  if (subjects.length === 0) {
    return (
      <p className="pb-20 text-muted">
        No domain topics have been published yet. Check back soon.
      </p>
    );
  }
