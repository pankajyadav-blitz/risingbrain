...
/**
 * Three escalating completion celebrations, all pure-CSS (no deps), rendered in a
 * single fixed portal overlay so they never affect layout and never block clicks:
 *
 *  - "pattern" (a subcategory cleared)  → a small green confetti burst.
 *  - "topic"   (a whole category done)  → a larger multicolour burst + a centred toast.
 ...
 * pattern AND the topic AND the sheet), the grandest tier wins.
 */

type Variant = "pattern" | "topic" | "sheet";

const RANK: Record<Variant, number> = { pattern: 1, topic: 2, sheet: 3 };
// How long the overlay stays mounted (ms) — must cover the longest animation.
const DURATION: Record<Variant, number> = { pattern: 1500, topic: 2200, sheet: 3600 };
...
export function CelebrationProvider({ children }: { children: React.ReactNode }) {
  const [active, setActive] = useState<{ variant: Variant; id: number } | null>(null);
...
function CelebrationOverlay({ active }: { active: { variant: Variant; id: number } | null }) {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  if (!mounted || !active) return null;
  // `key` remounts the burst each time so its CSS animations replay.
  return createPortal(<Celebration key={active.id} variant={active.variant} />, document.body);
}
